...
while True:
    ...
    if 9<hour<16:
        ...
        if not last_status:
            ...
            print(obj['Total Load Power(W)'] - last_status['Total Load Power(W)'])
            if obj['Battery Power(W)'] < last_status['Battery Power(W)']:
                # fatah techrij zyede l inverter
                print("automatic continue increase")
            else:
                if (abs(last_status['Battery Power(W)']) + obj['Battery Power(W)']) <=  (obj['Total Load Power(W)'] - last_status['Total Load Power(W)']):
